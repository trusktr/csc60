...
printf("### DEBUG: %i\n", 3);
	...
	for (i=0; i<numberOfChildren; i++) {
		fork_pids[i] = fork();
		outgoingPipes[i] = (int*)malloc( sizeof(int) * 2 );
...
/*sleep(5);*/
printf("\n\n### DEBUG: sizeof(outgoingPipes): %i\n", sizeof(outgoingPipes));
int x;
for (x=0; x<numberOfChildren; x++) {
	printf("    sizeof pipe #%i -->> %i\n", x, sizeof(outgoingPipes[x]));
	int y;
	for (y=0; y<2; y++) {
		printf("       fileno: %i\n\n", outgoingPipes[x][y]);
	}
